...
class MV23DPredictor(object):
    ...
    def predict(
        self, 
        input_imgs,  # ✅ fixed typo: 'intput_imgs' → 'input_imgs'
        save_dir="outputs/", 
        image_input=None,
        ...
        print("💥 Start CUDA for loading data...")
        torch.cuda.empty_cache()  # ✅ clear unused memory
        gc.collect()              # ✅ force Python garbage collection

        with torch.cuda.amp.autocast():
            data = self.load_data(input_imgs)

        print("💥 Start CUDA for generating 3D Mesh...")
        torch.cuda.empty_cache()  # ✅ clear memory again before next stage
        gc.collect()
